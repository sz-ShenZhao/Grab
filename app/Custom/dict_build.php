...
$dicAddon = dirname(__FILE__).'/dict/not-build/base_dic.txt';

if( empty($_GET['ac']) )
{
    echo "<div style='line-height:28px;'>请选择要进行的操作：<br />";
    echo "1、<a href='?ac=make'>用原始文件(dict/not-build/base_dic.txt)生成一个标准词典；</a><br />";
...
if( $_GET['ac']=='make' )
{
    ...
    $pa->MakeDict( $dicAddon,  16 , 'dict/base_dic.dic');
    echo "完成词典创建！";
    exit();
}
else
